...
class CompilationRequest {
     ...
     */
    constructor(contentHash, target, command, workdir, type) {
        this.target = target;
     ...
     */
    static createForText(text, command, type) {
        var userError = validateCommand(command);
        if (userError)
            return {request: null, userError};
        var hash = crypto.createHash('md5').update(text).digest("hex");
        var userError = null;
        var request = new CompilationRequest(hash, 'main.tex', command, type=type);
