...
class Compilation {
    ...
    async _innerRunCompilation() {
        ...
        if (!success) {
            this._finishCompilation(false, 'Compilation Job failed due to internal reasons');
            return;
        }
    ...
    _runShellScript(workdirPath, targetRelativePath, command, outputPath, logPath, timeout) {
        ...
        var shellProcess = spawn('bash', [compileScriptPath, workdirPath, targetRelativePath, command, '-shell-escape', outputPath, logPath]);
